...
output "ubuntu_docker_main_ips" {
  value = {
    for idx, nic in azurerm_network_interface.ubuntu_docker_main_nic : 
    "ubuntu-docker-main-${idx + 1}" => nic.ip_configuration[0].private_ip_address
  }
  description = "The private IP addresses of the Ubuntu VM instances and their labels."
}
